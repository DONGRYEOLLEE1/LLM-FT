...
def main(model_args, data_args, training_args):
    ...
    def process(x):
        ...
        return x
    data = load_dataset("json", data_files = "/data/askbiz/data/dbr_ft_data_10_lora.json")
    data = data.map(process, remove_columns = data['train'].column_names)
    tok_data = data.map(lambda x: tokenizer(x['text'], truncation = True, max_length = 2048), batched = True)
